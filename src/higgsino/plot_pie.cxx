...
int main(int argc, char *argv[]){
  ...
  NamedFunc base_func = HigUtilities::pass_2016 && "stitch";
  string higtrim = "hig_cand_drmax[0]<=2.2 && hig_cand_dm[0]<=40 && hig_cand_am[0]<=200";
  if (resolved) {
    base_func = base_func && "njet>=4 && njet<=5 && "+higtrim;
  } else {
    base_func = base_func && "nfjet>1 && fjet_pt[0]>300 && fjet_pt[1]>300 && ht>600";
    base_func = base_func && "fjet_msoftdrop[0]>50 && fjet_msoftdrop[0]<=250 && fjet_msoftdrop[1]>50 && fjet_msoftdrop[1]<=250";
  }
  ...
  //  Make pie table with Nb as columns and additional cuts as rows
  //-----------------------------------------------------------------
