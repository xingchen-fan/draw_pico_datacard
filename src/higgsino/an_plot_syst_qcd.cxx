...
int main(int argc, char *argv[]){
  ...
  string search_sig_skim_folder = "SMS-TChiHH_2D_fastSimJmeCorrection/unskimmed/";
  string ttbar_sig_skim_folder = "SMS-TChiHH_2D_fastSimJmeCorrection/unskimmed/";
  string zll_sig_skim_folder = "SMS-TChiHH_2D_fastSimJmeCorrection/unskimmed/";
  string qcd_sig_skim_folder = "SMS-TChiHH_2D_fastSimJmeCorrection/unskimmed/";
  ...
  if (HigUtilities::is_in_string_options(string_options, "paper_style")) {
    qcd_procs.push_back(Process::MakeShared<Baby_pico>("t#bar{t}+X", Process::Type::background,colors("tt_htau"),
    ...
    qcd_procs.push_back(Process::MakeShared<Baby_pico>("Other", Process::Type::background, kGray+2,
                    attach_folder(mc_base_folder, years, qcd_mc_skim_folder, mctags["other_and_single_t"]),"stitch"));
  }
  else {
    // Set mc processes
  ...
  pm.Push<Hist1D>(Axis(10, 0, 200, "hig_cand_am[0]", "<m_{bb}> [GeV]", {100, 140}),
    base_filters&&qcd_resolved&&"hig_cand_drmax[0]<1.1",
    qcd_data_procs_btag, plt_shapes_data).Weight(weight).Tag("FixName:syst__qcd_amjj_btag__lowdrmax__data").LuminosityTag(total_luminosity_string);
  pm.Push<Hist1D>(Axis(10, 0, 200, "hig_cand_am[0]", "<m_{bb}> [GeV]", {100, 140}),
    base_filters&&qcd_resolved&&"hig_cand_drmax[0]>=1.1",
    qcd_data_procs_btag, plt_shapes_data).Weight(weight).Tag("FixName:syst__qcd_amjj_btag__highdrmax__data").LuminosityTag(total_luminosity_string);
