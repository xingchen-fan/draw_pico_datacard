...
int main(int argc, char *argv[]){
  ...
  while(getline(infile, line_s)){
    ...
    double pmx, pmy, pxsec, pexsec, pobs, pexp, pup, pdown, p2up, p2down, sigobs, sigexp;
    iss >> pmx >> pmy >> pxsec >> pexsec >> pobs >> pexp >> pup >> pdown >> p2up >> p2down >> sigobs >> sigexp;
    vmx.push_back(pmx);
    vmy.push_back(pmy);
    vxsec.push_back(pxsec/hh4b_bf);
    vexsec.push_back(pexsec/hh4b_bf);
    vobs.push_back(pobs);
    vobsup.push_back(pobs);
    vobsdown.push_back(pobs);
    vexp.push_back(pexp);
